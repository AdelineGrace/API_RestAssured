...
Scenario: Check if user able to update a Batch with valid batchID and mandatory request body
 ...
 @Batch_TC_014
  Scenario: Check if user able to delete a Batch with valid programName

    Given User creates DELETE Request for the LMS API endpoint  and  valid programName
    When  User sends HTTPS Request with valid programName                                
    Then  User receives 200 OK Status with message.
  @Batch_TC_015
     Scenario: Check if user able to delete a Batch with valid LMS API,invalid programName

    Given User creates DELETE Request for the LMS API endpoint  and  invalid programName
    When  User sends HTTPS Request with invalid programName                                    
    Then  User receives 404 Not Found Status with message and boolean success details
